...
import { SAVE_USER_IMAGE, SAVE_USER_INFO } from "@/utils/Constant";
...
const SKILL_CATEGORIES = [
  ...
  },
  {
    ...
    skills: ["React", "Next.js", "Tailwind CSS", "Express", "Node.js"],
  },
  {
    name: "Languages",
  ...
  {
    name: "DevOps & Cloud",
    skills: ["Docker", "AWS", "Git", "Kubernetes", "Linux"],
  },
];
...
const Profile = () => {
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      setSelectedColor(
        typeof userInfo.colorIndex === "number" ? userInfo.colorIndex : 0,
      );
  ...
  const validateData = () => {
    ...
    if (!branch) {
      toast.error("Enter the branch");
      return false;
    }
    if (!batch) {
      toast.error("Enter the batch Number");
      return false;
    }
    return true;
  };
          ...
          <div className="bg-white rounded-[40px] shadow-2xl shadow-slate-200/60 border border-slate-100 flex flex-col lg:flex-row overflow-hidden min-h-[85vh]">
            ...
            <div className="lg:w-90 bg-slate-50/50 p-10 border-r border-slate-100 flex flex-col items-center">
              <div className="relative group mb-10">
                <Avatar.Root
                  className={`h-40 w-40 rounded-full overflow-hidden flex items-center justify-center border-8 border-white shadow-xl ${getColor(selectedColor)}`}
                >
                  {avatarSrc ? (
                    <Avatar.Image
                      src={avatarSrc}
              ...
              <div className="w-full space-y-10">
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest text-center block">
                    Theme Color
                  </label>
                  <div className="flex gap-2.5 flex-wrap justify-center">
                    {colors.map((c, i) => (
                      <div
                        ...
                        className={`h-7 w-7 rounded-full cursor-pointer transition-all border-2 ${selectedColor === i ? "border-slate-900 scale-110" : "border-transparent opacity-50"} ${c}`}
                      />
                    ))}
                  </div>
                </div>
            ...
            <div className="flex-1 p-8 md:p-12 space-y-10 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <Input
                      ...
                      disabled
                      className="h-14 rounded-2xl bg-slate-100/50 border-none px-6 text-slate-500 font-medium cursor-not-allowed italic"
                    />
                  </div>
                  <Input
                    ...
                    {role === "alumni" && (
                      ...
                      >
                        <Input
                          ...
                          className="h-14 rounded-2xl bg-blue-50/50 border-2 border-blue-100 text-blue-700 px-6 font-semibold"
                        />
                      </motion.div>
                    )}
                  </AnimatePresence>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  <h2 className="text-sm font-black uppercase text-blue-600 tracking-widest">
                    Expertise & Skills
                  </h2>
                ...
                <div className="flex flex-wrap gap-2 min-h-14 p-3 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                  {skills.length === 0 && (
                    <p className="text-slate-400 text-xs italic ml-2 mt-2">
                      Add skills below...
                    </p>
                  )}
                  {skills.map((s) => (
                    <motion.span
                      ...
                      animate={{ scale: 1 }}
                      key={s}
                      className="bg-blue-600 text-white px-4 py-1.5 rounded-xl text-[11px] font-bold flex items-center gap-2"
                    >
                      {s}{" "}
                      <IoMdClose
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
                  {SKILL_CATEGORIES.map((cat) => (
                    <div key={cat.name} className="space-y-3">
                      <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest border-b border-slate-100 pb-1">
                        {cat.name}
                      </h4>
              ...
              <div className="pt-6">
                <Button
                  onClick={saveChanges}
                  className="h-14 px-12 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-base shadow-xl shadow-blue-200 transition-all active:scale-95 block mx-auto md:mx-0"
                >
                  Complete Setup
                </Button>
              </div>
